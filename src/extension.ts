...
export function activate(context: vscode.ExtensionContext) {
    try {
        ...
        if (workspaceFolders && workspaceFolders.length > 0) {
            findReactNativeProject(workspaceFolders[0].uri).then(
                (rnPath) => {
                    if (!rnPath) {
                        ...
                        console.log(`RN Profiler AI: Found React Native project at: ${rnPath}`);
                    }
                }
            );
        }
 ...
 */
async function findReactNativeProject(workspaceUri: vscode.Uri): Promise<string | null> {
    const commonPaths = [
        '', // Root
        'apps/mobile',
        'apps/react-native',
...
async function updateRNConfig(enabled: boolean, components: string[]) {
    ...
    const configPath = vscode.Uri.joinPath(vscode.Uri.file(rnPath), 'src', 'rn-profiler-config.ts');
